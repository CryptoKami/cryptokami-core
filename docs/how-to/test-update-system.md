...
* Build cryptokami-sl with `devnet_shortep_full` (`./scripts/build/cryptokami-sl.sh --dn`)
...
```
stack exec -- cryptokami-auxx --system-start 0 --log-config log-config-prod.yaml --logs-prefix "logs/abc4" --db-path db-abc4 --peer {relayHost}:3000 repl
```
