...
* Generate *before* installer, use `devnet_shortep_wallet`
   * checkout from latest release branch (`cryptokami-sl-0.6-staging` for instance) of `daedalus`
   * set appropriate `system-start` in `Launcher.hs`, relay ip in `wallet-topology.yaml`
   * set appropriate `cryptokami-sl` branch in `.travis.yml`, `appveyor.yaml`
   * see https://github.com/CryptoKami/daedalus/commit/98998da298a9ff86e6dd4d1c18e09ba8e396c545 for reference
* Generate *after* installer, use `devnet_shortep_updated_wallet`
   * same as previous item
* Build cryptokami-core with `devnet_shortep_full` (`./scripts/build/cryptokami-sl.sh --dn`)
...
cd nodes2
for i in {0..4}; do stack exec -- cryptokami-keygen rearrange --mask testnet$i.key; done
```

### Cryptokami-auxx

Launch `cryptokami-auxx` with CLI:

```
stack exec -- cryptokami-auxx --system-start 0 --log-config log-configs/daedalus.yaml --logs-prefix "logs/abc4" --db-path db-abc4 --peer {relayHost}:3000 repl
```
...
44001     block: v0.0.0
44002     software: cryptokami-sl:0
...
Installer should be put by cryptokami-node to `%APPDATA%\Daedalus\Installer.exe`.
