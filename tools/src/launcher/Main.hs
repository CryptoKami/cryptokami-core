...
import           Paths_cryptokami_sl (version)
...
getLauncherOptions = do
  ...
  where
    ...
    programInfo = info (helper <*> versionOption <*> launcherArgsParser) $
        fullDesc <> progDesc ""
                 <> header "Tool to launch CryptoKami Core."
                 <> footerDoc usageExample

    versionOption = infoOption
        ("cryptokami-launcher-" <> showVersion version)
...
Command example:
  ...
  stack exec -- cryptokami-launcher --config launcher-config.yaml
...
writeWindowsUpdaterRunner runnerPath = liftIO $ do
    ...
    writeFile (toString runnerPath) $ unlines
        [ "TaskKill /IM cryptokami-launcher.exe /F"
        -- Run updater
        , "%*"
        -- Delete updater
        , "del %1"
        -- Run launcher again
        , "start \"cryptokami launcher\" /b " <> (quote $ toText exePath) <> " " <> (unwords $ map (quote . toText) launcherArgs)
        -- Delete the bat file
        , "(goto) 2>nul & del \"%~f0\""
        ]
...
spawnNode nd doesWalletLogToConsole = do
    ...
    cr <- liftIO $ case mLogPath of
        Just lp -> do
            ...
            -- TODO (jmitchell): Find a safe, reliable way to print `logPath`. Cryptokami
...
reportNodeCrash exitCode logConfPath reportServ = liftIO $ do
    ...
    bracket (compressLogs logFiles) removeFile $ \txz ->
        sendReport [txz] (RCrash ec) "cryptokami-node" reportServ
