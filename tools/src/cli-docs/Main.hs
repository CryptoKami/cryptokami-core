...
optionsParser = do
    pathToBinDirectory <- strOption $
        long    "bin-dir" <>
        help    "Path to the directory with CryptoKami Core executable files." <>
        metavar "PATH"
...
getCLIDocsOptions pathToMarkdownFile = execParser programInfo
  where
    programInfo = info (helper <*> versionOption <*> optionsParser) $
        fullDesc <> progDesc "Generate Markdown chapter for cryptokamidocs.com."
                 <> header "Tool to generate CLI-docs for CryptoKami Core executable files."
                 <> footer ("Assumed that this program will run on Travis CI. " <>
                            "Produced file '" <> pathToMarkdownFile <> "' will be " <>
                            "renamed in a chapter and pushed in cryptokamidocs.com repository.")

    versionOption = infoOption
        ("cryptokami-cli-docs-" <> showVersion version)
...
main = do
  ...
  where
    ...
    getExecutables binDir = filter cryptokamiExeOnly <$> listDirectory binDir
      where
        cryptokamiExeOnly exe = "cryptokami-" `isPrefixOf` exe
...
---
layout: default
title:  CryptoKami Core CLI Options
...
CryptoKami Core CLI Options
----------------------

This guide describes all executable files that are used in CryptoKami Core and all corresponding CLI-options/parameters.
|]
