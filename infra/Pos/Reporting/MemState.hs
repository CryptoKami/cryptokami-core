...
initializeMisbehaviorMetrics store = liftIO $ do
    _mmRollbacks <- Metrics.createGauge (withCryptokamiNamespace "BlockRollbacks") store
    _mmSscFailures <- Metrics.createCounter (withCryptokamiNamespace "SCCComputationFailures") store
    _mmIgnoredCommitments <- Metrics.createGauge (withCryptokamiNamespace "IgnoredCommitments") store
