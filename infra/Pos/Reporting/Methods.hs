...
import           Paths_cryptokami_sl_infra (version)
...
sendReport logFiles reportType appName reportServerUri = do
  ...
  where
    ...
    mkReportInfo curTime =
        ...
        -- We are using version of 'cryptokami-sl-infra' here. We agreed
        -- that the version of 'cryptokami-sl' and it subpackages should
        -- be same.
        , rVersion = version
    ...
    then onNoServers
        ...
        withPath $ \(logFile :: [FilePath]) -> do
            errors <-
                ...
                try . sendReport logFile reportType "cryptokami-node" . toString
...
reportOrLog severity prefix exc =
    case tryCast @CryptokamiFatalError <|> tryCast @ErrorCall <|> tryCast @DBError of
