...
-- The DSL and Cryptokami core have two different solutions to this:
--
-- * In the DSL a transaction can have an empty list of inputs
-- * In Cryptokami transaction lists transaction _hashes_ as their inputs, rather
...
--
-- In order to be able to compare the DSL or Cryptokami, we need to create an
-- initial "bootstrap" transaction that sets things up to mirror the Cryptokami
