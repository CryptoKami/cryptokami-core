-- | Interpreter from the DSL to Cryptokami types
...
-- DSL hashes to Cryptokami hashes.
...
--
-- NOTE: In Cryptokami there is no equivalent of the boot transaction and hence
...
{-------------------------------------------------------------------------------
  Translate the DSL UTxO definitions to Cryptokami types

  NOTE: Delegation in Cryptokami is described in the document
  "Delegation and Stake Locking in CryptoKami Core"
  <cryptokami-sl-articles/delegation/pdf/article.pdf>.
-------------------------------------------------------------------------------}
      ...
      mkBlock mPrev slotId ts = do
        ...
        prev <-
          case mPrev of
            ...
            Nothing   -> (Left  . view gbHeader) <$> asks (ccBlock0 . tcCryptokami)
