...
module UTxO.Translate (
    -- * Monadic context for the translation from the DSL to Cryptokami
    TranslateT
  , Translate
  , runTranslateT
  , runTranslate
...
{-------------------------------------------------------------------------------
  Testing infrastructure from cryptokami-sl-core
...
{-------------------------------------------------------------------------------
  ...
  dictionaries so that we can lift Cryptokami operations to the 'Translate' monad.
...
runTranslateT (TranslateT ta) =
    ...
    withDefUpdateConfiguration $
      let env :: TranslateEnv
          env = TranslateEnv {
                    teContext = initContext initCryptokamiContext
                  , teConfig  = Dict
                  , teUpdate  = Dict
                  }
...
verifyBlocksPrefix blocks = do
    CryptokamiContext{..} <- asks tcCryptokami
    let tip         = ccHash0
        currentSlot = Nothing
