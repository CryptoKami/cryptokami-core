...
-- | Context needed for the translation between DSL and Cryptokami types
module UTxO.Context (
    -- * Cryptokami provided context
    CryptokamiContext(..)
  , initCryptokamiContext
    -- * Actors
  , Actors(..)
  , Rich(..)
  , Poor(..)
...
{-------------------------------------------------------------------------------
  Summary of the information we get about the genesis block from Cryptokami core
-------------------------------------------------------------------------------}

-- | The information returned by core
data CryptokamiContext = CryptokamiContext {
...
initCryptokamiContext :: HasConfiguration => CryptokamiContext
initCryptokamiContext = CryptokamiContext{..}
  where
    ...
    ccSecrets  = fromMaybe (error "initCryptokamiContext: secrets unavailable") $
                 generatedSecrets
...
{-------------------------------------------------------------------------------
  Deriving 'Actors' from 'CryptokamiContext'
...
initActors :: CryptokamiContext -> Actors
initActors CryptokamiContext{..} = Actors{..}
  where
    actorsRich  :: Map PublicKey Rich
    actorsPoor  :: Map PublicKey Poor
...
-- | Mapping between our addresses and Cryptokami addresses
...
{-------------------------------------------------------------------------------
  ...
  and Cryptokami types. Accumulation of the environments above.
-------------------------------------------------------------------------------}

data TransCtxt = TransCtxt {
      tcCryptokami  :: CryptokamiContext
    , tcActors   :: Actors
    , tcAddrMap  :: AddrMap
    }

initContext :: CryptokamiContext -> TransCtxt
initContext tcCryptokami = TransCtxt{..}
  where
    tcActors  = initActors  tcCryptokami
...
leaderForSlot slotId TransCtxt{..} = actorsStake Map.! leader
  where
    Actors{..}         = tcActors
    CryptokamiContext{..} = tcCryptokami
...
instance Buildable CryptokamiContext where
  build CryptokamiContext{..} = bprint
      ( "CryptokamiContext"
      % "{ leaders:  " % listJson
      % ", stakes:   " % listJson
      % ", balances: " % listJson
