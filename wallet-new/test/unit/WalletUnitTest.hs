-- | Wallet unit tests
--
-- TODO: Take advantage of https://github.com/CryptoKami/cryptokami-core/pull/2296 ?
...
import qualified Pos.Block.Error as Cryptokami
import qualified Pos.Txp.Toil    as Cryptokami
...
-- ordinary address. This currently has no equivalent in Cryptokami, so we omit
-- it.
...
intAndVerify pc = runTranslateT $ do
    ...
    case intResult of
      ...
      Right (chain', ctxt) -> do
        ...
        isCryptokamiValid <- verifyBlocksPrefix chain''
        case (dslIsValid, isCryptokamiValid) of
...
data ValidationResult =
    -- | We expected the chain to be valid; DSL and Cryptokami both agree
    ExpectedValid

    -- | We expected the chain to be invalid; DSL and Cryptokami both agree
    --
    -- We record the error message we get from Cryptokami (the DSL just reports
    -- true or false). Note that some invalid chains cannot even be constructed
    -- (for example, when we try to overspend).
  | ExpectedInvalid (Either Cryptokami.VerifyBlocksException IntException)

    -- | Disagreement between the DSL and Cryptokami
    --
    -- This indicates a bug. Of course, the bug could be in any number of
    -- places:
    --
    -- * Our translatiom from the DSL to Cryptokami is wrong
    -- * There is a bug in the DSL definitions
    -- * There is a bug in the Cryptokami implementation
    --
    -- We record the error message from Cryptokami, if Cryptokami thought the chain
    -- was invalid
  | Disagreement Disagreement

-- | Disagreement between Cryptokami and the DSL
--
-- We consider something to be "unexpectedly foo" when Cryptokami says it's
-- " foo " but the DSL says it's " not foo "; the DSL is the spec, after all
-- (of course that doesn't mean that it cannot contain bugs :).
data Disagreement =
    -- | Cryptokami reported the chain as invalid, but the DSL reported it as
    -- valid. We record the error message from Cryptokami.
    UnexpectedInvalid Cryptokami.VerifyBlocksException

    -- | Cryptokami reported an error during chain translation, but the DSL
    -- reported it as valid.
  | UnexpectedError IntException

    -- | Cryptokami reported the chain as valid, but the DSL reported it as
    -- invalid.
  | UnexpectedValid

    -- | Both Cryptokami and the DSL reported the chain as valid, but they computed
    -- a different UTxO
  | forall h. Hash h Addr => UnexpectedUtxo {
        utxoDsl     :: Utxo h Addr
      , utxoCryptokami :: Cryptokami.Utxo
      , utxoInt     :: Cryptokami.Utxo
      }
