  ...
  where
    ...
    toGenesisAddressInfo (address, coin) = do
        ...
        pure CGenesisAddressInfo
            { cgaiCryptokamiAddress = toCAddress address
...
-- | Deserialize Cryptokami or RSCoin address and convert it to Cryptokami address.
...
cAddrToAddr cAddr@(CAddress rawAddrText) =
    ...
    in case mDecodedBase64 of
        Just addr -> do
            -- the decoded address can be both the RSCoin address and the Cryptokami address.
            -- > RSCoin address == 32 bytes
            -- > Cryptokami address >= 34 bytes
            if (BS.length addr == 32)
                then pure $ makeRedeemAddress $ redeemPkBuild addr
                else either badCryptokamiAddress pure (fromCAddress cAddr)
        Nothing ->
            -- cAddr is in Cryptokami address format or it's not valid
            either badCryptokamiAddress pure (fromCAddress cAddr)
  where

    badCryptokamiAddress = const $ throwM $ Internal "Invalid Cryptokami address!"
