...
-- | Detail page of a `genesis block` https://cryptokamidocs.com/technical/blocks/#genesis-block
...
import Pos.Explorer.Web.Lenses.ClientTypes (_CAddress, cgaiCryptokamiAddress, cgaiGenesisAmount, cgaiIsRedeemed, cgsNonRedeemedAmountTotal, cgsNumNotRedeemed, cgsNumRedeemed, cgsNumTotal, cgsRedeemedAmountTotal, getCoin)
...
addressInfosBodyView lang (CGenesisAddressInfo info) =
    let addrLink = toUrl $ Address (info ^. cgaiCryptokamiAddress)
        addrString = info ^. (cgaiCryptokamiAddress <<< _CAddress)
    in
