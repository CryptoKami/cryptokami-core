...
# USAGE
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   scripts/build/cryptokami-sl.sh                 build all packages one-by-one
#   scripts/build/cryptokami-sl.sh -t              build and run tests
#   scripts/build/cryptokami-sl.sh core|db|...     build only a specific project (see below)
...
#   :
#   core, db, etc.                  cryptokami-sl-{core,db,etc.}
#   lib                             cryptokami-sl
...
do
  ...
  # benchmarks config
    ...
    # We want:
    # • --flag cryptokami-sl-core:-asserts ($asserts)
...
if [[ $for_installer == true ]]; then
  commonargs="$commonargs --flag cryptokami-sl-tools:for-installer"
fi

if [[ $asserts == false ]]; then
  commonargs="$commonargs --flag cryptokami-sl-core:-asserts"
fi
...
if [[ $spec_prj == "" ]]; then
  ...
  to_build="cryptokami-sl-node cryptokami-sl-wallet"
elif [[ $spec_prj == "wallet-new" ]]; then
  to_build="cryptokami-sl-node cryptokami-sl-wallet-new"
elif [[ $spec_prj == "explorer" ]]; then
  to_build="cryptokami-sl-node cryptokami-sl-explorer"
