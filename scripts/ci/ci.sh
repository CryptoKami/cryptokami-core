...
if [[ "$OS_NAME" == "linux" ]]; then
   targets="$targets cryptokami-sl-explorer-static"
fi
...
#  to_build="$to_build cryptokami-sl-$prj"
#done
...
pushd daedalus
  ...
  cp ../cryptokami-sl-wallet.root/bin/cryptokami-node .
  # check that binaries exit with 0
  ./cryptokami-node --help > /dev/null
  ./cryptokami-launcher --help > /dev/null
popd
...
if [[ ("$OS_NAME" == "linux") ]]; then
  ...
  export EXPLORER_EXECUTABLE=$(pwd)/cryptokami-sl-explorer-static.root/bin/cryptokami-explorer-hs2purs
  ./explorer/frontend/scripts/build.sh

  echo "Packing up explorer-frontend ..."
  APP_NAME=cryptokami-sl-explorer
  mkdir -p ${APP_NAME}
