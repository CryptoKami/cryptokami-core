...
time for i in ${NODES[@]}; do
    ...
    stack exec --nix -- cryptokami-keygen $CONF_PARAMS generate-key --path "$key_path"
    if [[ ! -f "$key_path" ]]; then
        echo "File $key_path not created"
        exit 1
    fi
    echo "Created key: "
    stack exec --nix -- cryptokami-keygen $CONF_PARAMS read-key --path "$key_path"
    stack exec --nix -- cryptokami-keygen $CONF_PARAMS read-key --path "$key_path" \
...
for i in ${NODES[@]}; do
    key_path="${STAKEHOLDER_SECRET}/secret${i}.key"
    stack exec --nix -- cryptokami-keygen $CONF_PARAMS rearrange --mask "$key_path"
    stack exec --nix -- cryptokami-auxx --keyfile "$KEYFILE_PATH" $CONF_PARAMS cmd --commands "add-key $key_path"
done

stack exec --nix -- cryptokami-auxx --keyfile "$KEYFILE_PATH" $CONF_PARAMS cmd --commands 'listaddr'
nAddrs=$(stack exec --nix -- cryptokami-auxx --keyfile "$KEYFILE_PATH" $CONF_PARAMS cmd --commands 'listaddr' | grep 'addr: ' | wc -l)
...
for i in ${NODES[@]}; do
    ...
    dpk="${DPKS[$i]}"
    dump_line=$(stack exec --nix -- cryptokami-auxx --keyfile "$KEYFILE_PATH" $CONF_PARAMS cmd --commands "delegate-heavy $skn $dpk 0 dump" | grep -E 'JSON: key \S+, value')
