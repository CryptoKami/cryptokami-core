...
blacklistedPackages = [ "cryptokami-sl-auxx"
                      , "cryptokami-sl-tools"
                      ]

--------------------------------------------------------------------------------
-- Filter `cryptokami-sl-auxx` & `cryptokami-sl-tools` as they cannot be
...
unavoidableDeps myself x = and [
      x /= myself
    , not ("cryptokami" `T.isInfixOf` (fst x))
    ]

--------------------------------------------------------------------------------
-- | Filter "unavoilable" dependencies like the ones of the cryptokami family.
...
main = do
    ...
    let tableHeader         =  printf "%-40s" ("Package" :: String)
                            ...
                            <> printf "%-70s" ("Reverse deps (excluding cryptokami-*)"  :: String)
...
showRevDeps []  = T.pack $ printf "%-4d%s" (0 :: Int) ("(possibly cryptokami depends on it)" :: String)
